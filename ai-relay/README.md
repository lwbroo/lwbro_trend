...
跟 `oauth-relay/` 同一個道理：前端是純靜態頁，Anthropic API Key 不能放進前端
程式碼，所以獨立一個只做一件事的 Cloudflare Worker：收當日已經算好的八字／
紫微流運資料，丟給 Claude 生成白話的今日運勢段落，回傳。不落地、不記錄任何
...
1. **取得 Anthropic API Key**
   到 https://console.anthropic.com/settings/keys 建立一個新的 API Key。

2. **部署 Worker**
   ```bash
   cd ai-relay
   npx wrangler deploy
   npx wrangler secret put ANTHROPIC_API_KEY   # 貼上步驟 1 的 key
   ```
   部署完成後會印出一個網址，例如：
...
現在用的是 `claude-haiku-4-5`，一次解讀在 400 tokens 以內，成本很低，但這個
